...
cat eng/all_text.txt | python aligned_path_to_text.py 1 occ > aligned_sl_mt/spink_lewis.ref.txt

# Extract corresponding MT sentences from ChatGPT and Claude
cat chatgpt_occ/converted_all.txt | cut -f2 | python aligned_path_to_text.py 0 occ > aligned_sl_mt/chatgpt.mt.txt
cat claude_occ/sonnet_converted.txt | cut -f2 | python aligned_path_to_text.py 0 occ > aligned_sl_mt/claude.mt.txt
...
for mt_file in `ls occ_translations/$subdir`; do
    cat occ_translations/$subdir/$mt_file | cut -f2 | python aligned_path_to_text.py 0 occ > aligned_sl_mt/translations/$subdir/$mt_file
done
done
