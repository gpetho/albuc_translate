error_found=false; while IFS= read -r file; do [ "$(wc -l < "$file")" -ne 2806 ] && echo "File $file does not have exactly 2806 lines. It has $(wc -l < "$file") lines." && error_found=true; done < <(find aligned_sl_occ -type f); [ "$error_found" = false ] && echo "All files in the aligned_sl_occ directory and its subdirectories have exactly 2806 lines."
