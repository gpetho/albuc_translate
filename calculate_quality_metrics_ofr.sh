...
cat chatgpt_ofr/combined_new.txt | cut -f2 | python aligned_path_to_text.py 0 ofr > aligned_sl_ofr/chatgpt.mt.txt
cat claude_ofr/claude_ofr_new.txt | python aligned_path_to_text.py 0 ofr > aligned_sl_ofr/claude.mt.txt
...
find ofr_translations -name '*.txt' | sed 's/^ofr_//' | parallel -j+0 --bar '
    if [ ! -f sacrebleu_output_ofr/{}.json ]; then
        echo {}
